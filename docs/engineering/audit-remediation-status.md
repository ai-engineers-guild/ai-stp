...
| `RVR-P2-005` | `modified` | Wrong branch or source SHA. Deployment checks successful push-to-main provenance and exact checked SHA. Branch protection and dev-to-main promotion are governed by ADR-0180; historical issue `#188` is not an active requirement here. | Owner: repository operations. Preserve workflow/host separation checks and verify every promoted deployment. |
| `RVR-P2-006` | `confirmed` | Unmerged server contract. The server implementation is in the public product. Current regressions replace the obsolete transfer plan; restored `dev` is the integration branch under ADR-0180, not pending server transfer. | Owner: platform. Preserve current server regressions; no transfer remains. |
