...
This document is the disposition of the August 2026 RVR runner and deploy
audit (`RVR-P*`). It is not the architecture-alignment remaining-work list.
That list lives in
[`implementation-roadmap.md`](implementation-roadmap.md) under Architecture-alignment
dispositions. Do not enable GitHub branch protection on `ai-stp/main`
(`ADR-0115`). Publication is one `ai-stp-cli` wheel (`ADR-0146`). Issue `#188`
named below does not exist in this repository; the remaining protection question
is refused by `ADR-0115`.
...
| `#185 release` | `modified` | The unpublished candidate is one `ai-stp-cli` wheel and sdist (`ADR-0146`), with metadata, LICENSE, a deterministic CycloneDX SBOM, `release-manifest.json`, and `SHA256SUMS`. Historical five-pair comparison described the former internal packages. Build and OIDC attestation are separated by privileges and, from 2026-08-16, executed by different one-time workers (`ADR-0101`). Upload authority is the `pypi-cli` environment. | Obtain Linux clean-install evidence from the published `ai-stp-cli` wheel. Do not enable branch protection (`ADR-0115`). Publication of a later version remains a separate permission. Owner: release operations. |
