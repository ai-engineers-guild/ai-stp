...
| Released CLI | `0.0.27` on PyPI (`apps/cli/pyproject.toml` matches) |
...
3. **Next CLI cut** after qualify of clean bytes. `0.0.27` is on PyPI from #386/#387 and carries the completed `parameter_rules` vocabulary (#375, #380), the leaf-scoped parse-failure contract with the full missing-options set (#385), plus the Haiku 4.5 qualification record (#368).
...
| Released CLI then | `0.0.22`. Current PyPI is `0.0.27` |
...
`0.0.27` on PyPI is the current reader, and it carries the Haiku 4.5 qualification recorded in #368, the completed parameter_rules vocabulary (#375, #380), and the leaf-scoped parse-failure contract (#385).
