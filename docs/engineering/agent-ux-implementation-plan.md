...
| Released CLI | `0.0.25` on PyPI (`apps/cli/pyproject.toml` matches) |
...
3. **Next CLI cut** after qualify of clean bytes. `0.0.25` is on PyPI from #376/#377 and carries the machine-help contract corrections (#375) plus the Haiku 4.5 qualification record (#368).
...
| Released CLI then | `0.0.22`. Current PyPI is `0.0.25` |
...
`0.0.25` on PyPI is the current reader, and it carries the Haiku 4.5 qualification recorded in #368 plus the contract corrections from #375.
