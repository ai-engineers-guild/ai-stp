...
| `tests/unit/test_cli_publication.py`, `test_cli_private_distribution.py`, `test_cli_sync_transport.py`, `test_cli_grants.py`, `test_cli_reports.py`, `test_cli_artifact.py` | replace | `/v1`-mock journeys (publication plans, sync pull/push, grants, requests, artifact fetch); the routes exist in the real app — migration to `tests/api/cli/` pending |
