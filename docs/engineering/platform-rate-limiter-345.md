...
трассировка вытеснения ключей. Политика ключа «метод + шаблон + адрес»
заменена двумя окнами `ADR-0128`; вытеснение пустого окна и запрет общей
overflow-корзины остаются.

## Нормативная база

Вытеснение ключей принадлежит этому плану. Политика окон — `SPEC-010`
`REQ-1015`, `ADR-0128` и раздел «Ограничение частоты» в
