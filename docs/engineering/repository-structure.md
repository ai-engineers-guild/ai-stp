...
`apps/*` contains entry points and dependency assembly. `apps/platform` owns persistence, queue, object storage, and domain services. `packages/foundation` contains only genuinely cross-cutting primitives. `packages/passports` owns passport and revision models. `packages/assurance` owns author-attestation records. `packages/sources` adapts GitHub and bounded local paths and never grants verification or target-write authority. `packages/contracts` owns the `/v1` wire boundary: request and response models, shared conventions for headers and cursors, and the mapping from error codes to statuses. It contains no business rules and makes no network calls; `apps/api` must conform to it, not generate it. Provider protocol adapters live in `apps/cli` (`ai_stp_cli.provider`) and `provider-kit/`; object storage implementation lives in `apps/platform`.
