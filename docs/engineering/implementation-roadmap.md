...
Завершить вредоносные проверки, тренировки восстановления, документацию оператора и финальные выпускные доказательства каталога запуска: содержимое корпуса создаётся поэтапно с фазы 4 (владельцы гильдии делят ролевые семейства между собой с обязательным взаимным ревью), а здесь остаётся только его итоговая инвентаризация и доказательства. Первый выпуск блокируется полнотой продуктовых требований, полным сквозным доказательством Claude Code и Codex и укомплектованным первопартийным каталогом запуска по `release-evidence.md`. Вместе с первым релизом репозиторий становится публичным, и включается защита веток по `quality-gates.md`.

Кодовые барьеры deployment уже требуют отдельные `runner labels` и отпечатки
физических hosts, запрещают отмену `push-run`, проверяют закреплённую SSH identity,
хранят credentials только в `run-scoped` каталоге, принимают лишь разрешённый
`PR merge` и выполняют внешний `strict-TLS/SHA probe`. Это готовность workflow, а не
доказательство настройки: отдельные физические runners, protected environments и
отрицательная репетиция изоляции остаются внешним gate repository operations.
