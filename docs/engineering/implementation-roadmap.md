...
| A5j | Resume a drain interrupted after answering a question | The failed author answer left a running row with no continuation, and the native agent repeatedly read its unchanged status. Share interruption recovery between answer and continue, expose the running revision's continuation on read, and verify that recovery after persistence reuses the immutable setup. Retain and resume the original live failure instead of resetting the registry |
| A5k | Explain the task input boundary on failed reads | A fresh native GPT OSS 120B session repeatedly supplied inline JSON to the file-based input option. Preserve the file-or-stdin contract, explain it on read failure, omit raw rejected input from details, and return scoped help. Invalid UTF-8 is a validation refusal too. Retain the original attempts separately from corrected execution |
| A5l | Await the harness shell result before interpreting CLI state | Native Antigravity yielded a shell task handle after three seconds while loaded-host CLI calls took longer. The agent confused shell tasks with CLI tasks, repeated commands and reported unobserved success. Clarify the control Skill's output-awaiting rule in both locales, refresh through the supported delivery path and repeat in a fresh native session; keep intentional backgrounding and model report failures visible |
| A5m | Install and change exact owner-local setup pins | The fresh session eventually supplied a real input file, then received a public-catalog 404 for its locally derived setup. Reuse exact current-owner records before catalog acquisition, retain the normal verified plan and provider path, and resolve authored source members from their exact embedded records. A partial pin must ask for the missing exact reference instead of silently selecting the baseline. Test ownership/version negatives and author-to-change-to-remove, then repeat exact native installation |
| A5n | Preserve explicit account sync authority and receipts | Native Herdr/agy in an already authenticated test home reproduced USER_DECISION_REQUIRED for an explicit account pull. Forward task authority to the sync service, retain typed push/pull receipts and honest goal status, and continue progressing pages within the same durable task. Stop on empty partial data or an unchanged cursor. Exercise real transport plus conflict/partial/replay regressions, then repeat the live account flow |
| A5o | Select a real account-sync entity | Native push attempted to send a local project passport and correctly hit the absolute-path guard; even a redacted root has an unsupported entity kind. Account push now requires a syncable `stable_id`, keeps project data local, and retains the exact disabled-sync configuration repair. Verify a task-driven API round trip to another device and repeat the native setup transfer |
| A5p | Install an exact account-synced immutable setup | Live second-device installation reached catalog 404 because sign-in changed the local owner while immutable authored snapshots retained theirs. Accept an exact version binding from an accepted event for the current account; keep foreign-account, pending-event and unmatched-version refusals, then repeat the real provider installation |
| A5q | Retain authored embedded material for immediate installation | Native direct installation of a freshly authored setting failed because the definition carried embedded bytes that were absent from CAS and the exact component graph. Retain validated members atomically at compose apply; repair old authored records on replay or local acquisition without rewriting versions. Preserve corrupt-byte refusals and test immediate skill/setting compilation. Distribution to another device still requires artifact acquisition, not passport-only sync |
| A5r | Route setup publication through the graph publication service | Native publish sent a setup id to the component-only service and refused with unavailable component revision. Dispatch setup ids to the existing publication set, checkpoint its exact digest in the task before confirmation, retain the typed set/member receipt, and preserve private visibility and replay idempotence. Retest through native Herdr/agy |
| A5s | Keep embedded members embedded through change | Native private publication exposed a derived graph that silently turned an authored member into a catalog pin. Preserve the exact frozen record and validate it again; do not rewrite old versions or bypass owner checks. Regression verifies the derived member is excluded from catalog publication plans |
| A5t | Validate and explain embedded publication adaptations | Real private publication passed embedded integrity and graph checks but failed exact adaptation because the check only loaded catalog rows. Inspect the bound definition's exact embedded passports, preserving digest, identity and harness guards. Publication-set receipts retain server evidence and transport error codes so a native agent can diagnose the result |
