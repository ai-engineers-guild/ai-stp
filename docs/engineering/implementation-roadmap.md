...
несовпадающем. Provider-половина принадлежит закрытому контуру авторинга
setup systems: `0.0.49` добавляет эхо обеим формам ответа, после чего оно
начинает проверяться реально, а не толерироваться.
...
Provider-половина выпущена как `0.0.49`. `evidence-software` против неё: 7/7
`passed`, `clean`, и `plan_digest` присутствует в **21 из 21** программного
результата против нуля из 21 у `0.0.48`. Толерантность перестала быть несущей:
присутствующее эхо сверяется с дайджестом плана, и все 21 совпали.
...
**Измерено, что это не блокер.** Полная сборка корпуса по всем 7×4 даёт
`unrouted` пустым: 28 строк, 71 компонент, ни одного ненаправляемого пути. То
есть шесть объявленных scope не стоят сегодня ни одного компонента, и
переимпорт каталога ими не заблокирован. Спецификация решает вопрос
**размещения**, а не наличия.
...
1. `just evidence-software <tag>` ведёт семь выпущенных провайдеров через
   ...
   `0.0.48` и `0.0.49` на Linux `x86_64` выполнен: 7/7 `passed`, `clean`. На
   остальных пяти нативных строках — нет.
