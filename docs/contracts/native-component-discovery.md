...
Каркас `component scaffold` — это **источник** с `managed_paths`, а не нативная
раскладка. Он проходит путь версии и пакета, а не приёмки: положить его в
нативный каталог и ждать `adopt` можно только для тех видов, чей манифест входит
в набор выше.
