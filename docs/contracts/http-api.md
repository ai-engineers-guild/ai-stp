...
Один узел считает бюджет отдельно на пару HTTP-метода, шаблона совпавшего маршрута и адреса клиента. Конкретный путь с идентификатором объекта или версии в ключ не входит: иначе обычный просмотр каталога исчерпывает таблицу. Ключ, у которого окно запросов уже пусто, освобождает слот. Новые посетители не сходятся в одну общую overflow-корзину. Превышение отвечает `AI_STP_RATE_LIMITED`. Распределённый лимитер не обещается.
