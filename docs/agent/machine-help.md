...
Класс изменения (`read`, `plan`, `apply`, `destructive`) и правило подтверждения (`none`, `explicit_flag`, `plan_digest`) объявляются на команде и определяют, обязан ли агент спросить пользователя до вызова. Правила поведения принадлежат `interaction-policy.md`, словарь значений — `packages/contracts`.
