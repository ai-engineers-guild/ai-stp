...
Каталог запуска первопартийный по `ADR-0034`: гильдия публикует из подтверждённого пространства имён базовые сетапы поддержанных харнессов и ролевые семейства для Claude Code и Codex; барьер запуска — пять базовых сетапов, и корпус его перерос. Публикации пользователей дополняют каталог, но холодный старт от них не зависит.
