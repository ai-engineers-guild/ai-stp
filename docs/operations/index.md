---
description: "Platform and CLI operations: configuration, infrastructure, CI, observability, and runbooks."
last_verified: "2026-08-03"
---

# Operations

<!-- CONTENTS: generated by just docs-gen; do not edit manually -->

| Документ | О чём | Сверено |
| -------- | ----- | ------- |
| [runbooks/](runbooks/index.md) | Step-by-step recovery for critical CLI and platform operations. | 2026-08-03 |
| [ci-cd.md](ci-cd.md) | Checks and release order for compatible changes. | 2026-08-25 |
| [configuration.md](configuration.md) | Configuration of the local CLI and server environment. | 2026-08-29 |
| [observability.md](observability.md) | Minimum signals for diagnosing the CLI, sync, publishing, and providers. | 2026-08-29 |
| [production-readiness.md](production-readiness.md) | Operator procedure for evidence-gated production readiness and optional OpenObserve. | 2026-08-28 |
| [provider-integration-state.md](provider-integration-state.md) | Public compatibility snapshot for seven provider systems and ai_stp. | 2026-08-31 |

<!-- END CONTENTS -->
