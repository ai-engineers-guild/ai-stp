...
Живой индекс на 2026-08-29: все пять проектов опубликованы как `0.0.9`, проверено
чистой установкой `uv tool install --no-cache ai-stp-cli` без пина в пустом `HOME`,
заверение скачанного колеса называет репозиторий, `release-candidate.yml`,
`refs/tags/v0.0.9` и коммит `0e294917`.

**Увидеть версию в простом индексе — ещё не значит, что её возьмут.** На выпуске
`0.0.9` все пять проектов уже перечисляли `0.0.9` в `pypi.org/simple/<name>/`,
а чистая установка без пина в ту же минуту поставила `0.0.8` — весь замыкающий
набор. Через несколько минут та же команда в новом пустом `HOME` взяла `0.0.9`.
`--no-cache` тут не помогает: он отключает кэш `uv`, а расходится CDN, поэлементно
по краям. Поэтому проверкой считается **установка**, а не чтение индекса: индекс
отвечает раньше, чем резолвер видит то же самое.
