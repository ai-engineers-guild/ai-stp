...
organization. The wakeup invokes `heartbeat tick --organization <id>` through
the installed Python interpreter and a small local launcher that restores the
enrolled XDG config/data directories. The tick calls the same SQLite due
claim, organization policy lookup, fresh report assembly, and bounded retry
...
Windows uses a Task Scheduler task under the interactive user with missed-run
catch-up; macOS uses a LaunchAgent with an hourly calendar trigger and
...
The CLI reports the local subscription, scheduler registration, and recent
