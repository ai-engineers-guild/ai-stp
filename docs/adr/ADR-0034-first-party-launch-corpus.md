...
```text
базовые сетапы — по одному для каждого из пяти харнессов:
  Claude Code, Codex, Pi, OpenCode, Grok Build

ролевые семейства для Claude Code и Codex — шесть на каждый,
отдельными харнесс-специфичными сетапами:
...
Бета-паритет ролевых семейств для Pi, OpenCode и Grok Build выпуск не блокирует: их базовые сетапы остаются бета-линиями.
