...
**Чего это не даёт.** Ни один из семи не закреплён в `provider-policy.toml`:
релизов нет. До них установка идёт через `--unverified-provider` с записью
`provider_release_trusted: false`. Тир и состояние независимы по `REQ-3307`,
поэтому `beta` и `not_verified` сосуществуют честно.
