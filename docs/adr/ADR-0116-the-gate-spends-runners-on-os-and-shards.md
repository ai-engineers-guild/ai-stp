...
`contract`, `property`. Каждая job берёт восемь xdist-процессов на стандартном
четырёхъядерном раннере: тесты ждут базу и процессы, ядра не являются
узким местом, экономить runner-минуты здесь запрещено явно.
