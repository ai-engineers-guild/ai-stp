...
Статус: принято. Заменяет `ADR-0099` в части постоянного GitHub Actions runner.

## Контекст

`ADR-0099` правильно развернул транспорт: production-хост устанавливает
...
сервера. Для NDDev Drakkars это последнее постоянное Linux-исключение.
