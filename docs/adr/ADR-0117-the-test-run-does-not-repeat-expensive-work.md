...
Статус: принято. Уточняет `ADR-0104` в части стоимости прогона. Решение
распределять тесты по процессам и держать порог покрытия неизменным не
меняется.

## Контекст

После снятия цепочки между job (`ADR-0104`) стоимость прогона определяли два
повтора одной и той же работы, и оба были видны в коде до всякого замера.
...
Замер ADR-0104 уже показал, что сбор покрытия стоит около трети времени гейта
(787.89 с против 547.92 с без него), но сам бэкенд не выбирался.
