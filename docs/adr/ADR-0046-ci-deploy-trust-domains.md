...
Статус: принято; разделение trust domains действует полностью и от яруса staging никогда не зависело — цель развёртывания переименована в `ADR-0084`. Механизм роли CI пересмотрен в `ADR-0080` после перехода на
полностью эфемерные проверки. Разделение доменов доверия и вся часть о
развёртывании действуют без изменений.
...
Первая половина этого условия наступила: `ADR-0080` перевёл проверки на эфемерный
runner scale set. Часть о развёртывании остаётся открытой — оно по-прежнему держит
долгоживущий SSH-ключ на постоянной машине.
