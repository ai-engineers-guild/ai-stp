...
Перечислено, а не вспомнено: из 136 объявленных команд 24 требуют
подтверждения — 21 через `explicit_flag`, 3 через `plan_digest`. Двадцать из
двадцати одной оправданы списком выше: `device reset`, публикация компонента и
сетапа, гранты, отчёты, телеметрия, `install apply`, замена и усыновление
провайдера, подпись аттестации, `sync`, удаление инструмента и харнесса.
...
`register_graph` — digest предложения.
...
Решение: там, где операция локальна, обратима и уже требует точное ожидаемое
значение, подтверждением является оно. Отдельный `--confirm` у этих четырёх
снят, `confirmation` у них становится `plan_digest`. Там, где операция выходит
наружу или необратима, остаются оба.
