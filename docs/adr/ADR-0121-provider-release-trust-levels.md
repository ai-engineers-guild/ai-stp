...
- старые закрепления `NDDev-it-com/nddev-*-app` удаляются как вытеснённый эстейт; семь действующих setup-system разрешаются только отдельными build-attestation rules;
