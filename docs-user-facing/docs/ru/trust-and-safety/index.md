...
долговременная запись на одного publisher или одну мажорную линию объекта.
...
`--scope` — `publisher` или `object_major`. Более широкой формы нет. У цели
уже должны быть зарегистрированные объекты: пустой fingerprint — не согласие,
а отсутствие наблюдения.
...
Запись перестаёт покрывать версию, если этой версии нужны новые права,
процессы, сеть, учётные данные или нативные поверхности по сравнению с
fingerprint на момент согласия. Отзыв действует сразу для последующих
запросов.
