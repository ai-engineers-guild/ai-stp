---
title: "mcp"
description: "MCP-компоненты: серверы, которые отдают tools, и клиентский конфиг, который их называет."
---
...
([`skill`](skill.md) или [`instruction`](instruction.md)), «какой пакет
расширяет харнесс?» ([`plugin`](plugin.md)) и «какой именованный shortcut
набрать?» ([`command`](command.md)).
...
[Проверки безопасности](../security-checks.md). Для MCP ожидайте как
минимум:
...
[Компоненты](index.md), [`plugin`](plugin.md), [`setting`](setting.md).
