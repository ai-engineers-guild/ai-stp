---
title: "agent"
description: "Agent-компоненты: именованные роли и специализированные subagents внутри сетапа."
---
...
Он не отвечает «как эта роль должна делать работу?» ([`skill`](skill.md)),
«какие постоянные правила относятся ко всем?»
([`instruction`](instruction.md)) и «какой пакет расширяет харнесс?»
([`plugin`](plugin.md)).
...
[Проверки безопасности](../security-checks.md). Для agent ожидайте как
минимум:
...
[Компоненты](index.md), [`instruction`](instruction.md),
