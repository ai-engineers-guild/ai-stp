...
конкретному локальному графу сетапа или ко всем объявленным адаптациям одной
версии компонента и выполняет локальные детерминированные проверки. Он не
устанавливает, не публикует и не обращается к API модели.
...
| `ai-stp eval plan` | `plan` | `none` | привязать этот профиль к адаптациям харнесса этого сетапа |
| `ai-stp eval component plan` | `plan` | `none` | привязать этот профиль ко всем адаптациям одной версии компонента |
| `ai-stp eval run` | `apply` | `plan_digest` | выполнить локальные детерминированные проверки для одного подтверждённого плана сетапа |
| `ai-stp eval component run` | `apply` | `plan_digest` | выполнить локальные детерминированные проверки для одного подтверждённого плана компонента |
| `ai-stp eval component status` | `read` | `none` | прочитать неизменяемый статус одного локального прогона оценки компонента |
| `ai-stp eval component show` | `read` | `none` | показать полные неизменяемые локальные свидетельства одного прогона оценки компонента |
| `ai-stp eval status` | `read` | `none` | прочитать неизменяемый статус одного локального прогона оценки сетапа |
| `ai-stp eval show` | `read` | `none` | показать полные неизменяемые локальные свидетельства одного прогона оценки сетапа |

`--json` — глобальный флаг. Всегда передавайте его. `eval run` и
`eval component run` требуют `--expected-plan-digest`. Флага `--confirm` не
существует. План сетапа включает только адаптации харнесса этого сетапа.
План компонента включает каждую объявленную адаптацию версии.
