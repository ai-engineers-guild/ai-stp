---
title: "instruction"
description: "Instruction components: standing rules, memory and textual constraints for a harness."
---
...
([`skill`](skill.md)), "which named shortcut do I type?"
([`command`](command.md)), or "which specialised role should run?"
([`agent`](agent.md)).
...
!!! warning "AGENTS.md is an instruction, not an `agent`"
    ...
    `instruction` (or of [`setting`](setting.md)). There is no `memory`
    kind.
...
on [Security checks](../security-checks.md). For an instruction, expect
at least:
