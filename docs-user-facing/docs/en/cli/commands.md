...
| publication plan | `plan` | `none` | [publication.md](publication.md) | Expert: create an immutable server plan for one exact released component version. |
...
| publication confirm | `apply` | `explicit_flag` | [publication.md](publication.md) | Expert: confirm one exact unexpired publication plan hash. |
...
| auth complete | `apply` | `none` | [auth.md](auth.md) | Expert: finish the pending sign-in once the user has approved it. |
| auth login | `apply` | `none` | [auth.md](auth.md) | Expert: start a sign-in and report the code the user must approve. |
| auth logout | `apply` | `none` | [auth.md](auth.md) | Expert: end the cloud session on the server and here, keeping all local data. |
...
| component discover | `read` | `none` | [component-discover.md](component-discover.md) | Expert: list native components in one project or the harness roots. Changes nothing. |
...
| component scaffold plan | `plan` | `none` | [component-discover.md](component-discover.md) | Preview exact files and digests for one versioned component scaffold. |
| component scaffold apply | `apply` | `plan_digest` | [component-discover.md](component-discover.md) | Create exactly the confirmed component scaffold without overwriting a path. |
...
| component publish | `plan` | `none` | [component-publish.md](component-publish.md) | Extract one embedded component into the ordinary publication plan. |
...
| component adopt | `apply` | `none` | [component-discover.md](component-discover.md) | Expert: register one discovered component in the local registry. |
...
| config init | `apply` | `none` | [config.md](config.md) | Create the configuration file if it is absent, and validate it either way. |
...
| project passport | `apply` | `none` | [project.md](project.md) | Record a project passport revision pinning the index, toolchain and config. |
...
| registry acquire | `apply` | `none` | [registry.md](registry.md) | Acquire one exact published setup graph for local offline compilation. |
| registry port discover | `read` | `none` | [registry.md](registry.md) | Find compatible SX and APM snapshots under one explicit local root. |
| registry port inspect | `read` | `none` | [registry.md](registry.md) | Inspect one setup-store mapping without importing or running its CLI. |
| registry port plan | `plan` | `none` | [registry.md](registry.md) | Preview a local-only setup-store import and bind it to exact manifest bytes. |
| registry port import | `apply` | `plan_digest` | [registry.md](registry.md) | Import a confirmed exact SX or APM snapshot into the local registry only. |
...
| registry search | `read` | `none` | [registry.md](registry.md) | Search the public catalogue without an account. |
...
| select eligibility | `read` | `none` | [select.md](select.md) | Expert: which candidates a harness may be composed from, and why each refusal happened. |
| select eligibility-matrix | `read` | `none` | [select.md](select.md) | Where one object may be composed, answered for every supported harness. |
...
| select propose | `plan` | `none` | [select.md](select.md) | Expert: record one composition proposal. Creates no version and no target. |
| select confirm | `apply` | `none` | [select.md](select.md) | Expert: freeze one proposal as a private setup version, its trace and its pin. |
...
| select bundle | `read` | `none` | [select.md](select.md) | Compile the deterministic package for one composition. Writes to no target. |
| install plan | `plan` | `none` | [install.md](install.md) | Expert: compute an immutable installation plan. Has no effect of its own. |
| install approve | `apply` | `plan_digest` | [install.md](install.md) | Expert: approve one plan by its exact digest. Nothing else approves it. |
| install apply | `apply` | `plan_digest` | [install.md](install.md) | Expert: carry out one approved plan through its provider and record what happened. |
...
| `ai-stp sync preview` | `read` | `none` | [sync.md](sync.md) | Expert: preview local fast-forward, merge or conflict without changing a head. |
| sync push | `apply` | `explicit_flag` | [sync.md](sync.md) | Expert: push one exact local head with a durable replay-safe event. |
| `ai-stp sync merge` | `apply` | `explicit_flag` | [sync.md](sync.md) | Expert: commit a mechanically clean merge of two developer-passport heads. |
| sync pull | `apply` | `explicit_flag` | [sync.md](sync.md) | Expert: pull and atomically apply one bounded page from the account stream. |
...
| `ai-stp task intents` | `read` | `none` | [index.md](index.md) | List shipped intents. Start here. |
| `ai-stp task start` | `apply` | `none` | [index.md](index.md) | Create and drain one closed intent. |
| `ai-stp task answer` | `apply` | `none` | [index.md](index.md) | Supply one typed answer to an open question. |
| `ai-stp task continue` | `apply` | `none` | [index.md](index.md) | Advance one in-process step. |
| task status | `read` | `none` | [index.md](index.md) | Read one durable task. Changes nothing. |
| `ai-stp task cancel` | `apply` | `none` | [index.md](index.md) | Abandon a task that has not settled. |
| setup compose plan | `plan` | `none` | [setup.md](setup.md) | Expert: resolve and freeze a new setup from exact catalog, Git, package and local sources. |
| setup compose apply | `apply` | `plan_digest` | [setup.md](setup.md) | Expert: record the exact still-current mixed setup as one immutable local version. |
| setup import inspect | `read` | `none` | [setup.md](setup.md) | Read one native configuration and report what it holds. Writes nothing. |
| setup import plan | `plan` | `none` | [setup.md](setup.md) | Plan exact component and setup drafts from one native configuration. |
| setup publish plan | `plan` | `none` | [setup.md](setup.md) | Plan the publication of one released setup together with every component it pins. |
| setup publish confirm | `apply` | `explicit_flag` | [setup.md](setup.md) | Confirm one exact reviewed publication set: pinned components, then the setup. |
| setup update plan | `plan` | `none` | [setup.md](setup.md) | Preview replacing one embedded component with a newer exact snapshot. |
| setup update apply | `apply` | `plan_digest` | [setup.md](setup.md) | Apply one exact embedded update and create a new immutable setup version. |
| setup import register | `apply` | `plan_digest` | [setup.md](setup.md) | Register an inspected configuration as your own setup. No secret value is stored. |
...
| provider reinstall plan | `read` | `none` | [provider.md](provider.md) | Describe re-installing one exact provider version into the same path. Changes nothing. |
| provider reinstall apply | `apply` | `plan_digest` | [provider.md](provider.md) | Carry out exactly the provider reinstallation a plan described. |
...
| provider network | `read` | `none` | [provider.md](provider.md) | Report observed protocol-v2 network isolation on this machine. |
