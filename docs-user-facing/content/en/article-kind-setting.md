...
slug: kind-setting
locale: en
title: "A setting is non-secret behavioural configuration"
description: "An expanded guide to setting: config files, scope, MCP names, and the no-secrets rule."
published_at: 2026-09-04
tags: [component, setting]
draft: false
---

# `setting`

![Component type: setting](/content/illustrations/kind-setting.jpg)
...
([`instruction`](https://ai-stp.aiguild.space/en/docs/components)), "what must run on an event?"
([`hook`](https://ai-stp.aiguild.space/en/docs/components)), or "which MCP servers are declared in this same
file?" (that finding is kind [`mcp`](https://ai-stp.aiguild.space/en/docs/components), `native_role`
`mcp_client_config`).
...
on [Security checks](https://ai-stp.aiguild.space/en/docs/components). For a setting, expect at
least:
...
3. If the file also declares MCP servers, treat that as a separate
   [`mcp`](https://ai-stp.aiguild.space/en/docs/components) finding. Do not put server values in this artifact.
...
8. `component passport validate` → `component version release` to mint
   immutable `X.Y`.
9. Publish through [the publication path](https://ai-stp.aiguild.space/en/docs/components). In a
   setup, pin that `X.Y`.

Related: [Authoring](https://ai-stp.aiguild.space/en/docs/components),
[Components](https://ai-stp.aiguild.space/en/docs/components), [`mcp`](https://ai-stp.aiguild.space/en/docs/components), [`instruction`](https://ai-stp.aiguild.space/en/docs/components).
