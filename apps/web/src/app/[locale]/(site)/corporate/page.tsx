  ...
  return (
    <div className="min-w-0 space-y-8">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <CorporateList title={t("projects")} empty={t("noProjects")} items={context.projects} />
        <CorporateList title={t("teams")} empty={t("noTeams")} items={context.teams} />
      </div>
...
function CorporateList({
  ...
  title: string;
  empty: string;
  items: ReadonlyArray<{ name: string; state: string }>;
}) {
  return (
    <section className="border-border bg-card rounded-lg border p-5 shadow-sm sm:p-6">
        ...
        <ul className="mt-4 space-y-2">
          {items.map((item) => (
            <li key={item.name} className="border-border flex justify-between rounded border p-3">
              <span>{item.name}</span>
              <span className="text-muted-foreground text-sm">{item.state}</span>
            </li>
          ))}
        </ul>
      ) : (
