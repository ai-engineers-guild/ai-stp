...
type CorporateDetailMutationResult =
  { ok: true; data: unknown } | { ok: false; message: string; code?: ApiErrorCode };
...
export async function updateCorporatePresentationAction(input: {
  ...
  if (!resource.success) return { ok: false, message: "invalid corporate target" };
  let path: string;
  try {
  ...
  if (!parsed.success) return { ok: false, message: "invalid corporate profile" };
  try {
