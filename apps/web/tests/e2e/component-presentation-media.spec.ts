 ...
 */
for (const [objectKind, stableId] of [
  ["component", FIXTURE_COMPONENT_ID],
  ["setup", FIXTURE_SETUP_ID],
] as const) {
  test.describe(`${objectKind} presentation media editor`, () => {
    ...
    test("uploads image media, shows preview, and saves presentation", async ({ page }) => {
      ...
      await expect(
        page.getByRole("heading", {
          name: /Edit public presentation|Edit bio and media|\u0418\u0437\u043c\u0435\u043d\u0438\u0442\u044c \u0431\u0438\u043e \u0438 \u043c\u0435\u0434\u0438\u0430/i,
        }),
      ).toBeVisible();
    ...
    test("persists the full biography into the public catalog", async ({ page }) => {
    ...
    test("shows upload failure and recovery without treating preview as saved", async ({
      page,
    }) => {
      await page.goto(`/en/objects/${objectKind}/${stableId}/edit`);
      await selectUploadSource(page);
      ...
      let postCount = 0;
      await page.route(`**/api/objects/${objectKind}/${stableId}/media`, async (route) => {
