...
def _view(response: AccessVersionResponse, *, source: str, checked_at: str) -> CatalogVersionView:
...
def version(
    ...
    with client.open_client(endpoint, access_token=held.access_token) as http:
        response = client.call(
            http, "GET", path, AccessVersionResponse, attempts=endpoint.max_attempts
        )
...
def cached_version(
    ...
    response = AccessVersionResponse.model_validate(entry.document)
    if (response.kind, response.stable_id, response.version) != (kind, stable_id, number):
        raise CliFailure(
