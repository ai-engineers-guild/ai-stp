"""Install application service. Click handlers and the task engine share this.

One function per step the contract makes separate: plan, approve, apply, the two
...
PROVIDER_TRUST_KEYS: Final[tuple[str, ...]] = (
    "provider-manifest",
        ...
        invoke = invocation.provider_invoker(
            ...
            **_native_companion_access(
            ...
            unisolated_reason=trust.unisolated_reason(
                ...
                {
                    **parameters,
                    **{key: parameters[key] for key in PROVIDER_TRUST_KEYS if key in parameters},
                },
                recorded_trust=held.provider_release_trust,
            ),
        )
...
def _finish_mutation(view: InstallationView) -> Answer[InstallationView]:
    """Apply/resume answer: unmet install goals are errors, not `ok`."""
    if view.state in {"partial", "applied_unverified"}:
        raise CliFailure(
            "AI_STP_PARTIAL_OPERATION",
            "the requested install did not complete and needs recovery",
            details={"operation_id": view.operation_id, "state": view.state},
            operation_id=view.operation_id,
            continuations=[
                Continuation(
                    kind="retry",
                    path=["install", "recover"],
                    arguments={"operation": view.operation_id},
                    argv=[
                        "install",
                        "recover",
                        "--operation",
    ...
    if view.state == "cancelled":
        raise CliFailure(
            "AI_STP_CONFLICT",
            "the requested install was cancelled",
            details={"operation_id": view.operation_id, "state": view.state},
            operation_id=view.operation_id,
        )
    if view.state in {"failed", "stale"}:
        raise CliFailure(
            "AI_STP_PRECONDITION_FAILED",
            "the requested install did not complete",
            details={"operation_id": view.operation_id, "state": view.state},
            operation_id=view.operation_id,
        )
    return Answer(view, operation_id=view.operation_id)
