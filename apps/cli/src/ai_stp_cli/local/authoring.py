...
def add_adaptation(root: Path, harness: str) -> dict[str, bytes]:
    """Render one extra concrete harness projection beside the existing tree.

    The original `harness_variant` is unchanged. Generated bytes go under
    `projections/<harness>/` and are never the source of truth.
    """
    return add_adaptations(root, (harness,))


def add_adaptations(root: Path, harnesses: tuple[str, ...]) -> dict[str, bytes]:
    """Render extra concrete harness projections, or fail before writing any."""
    import json

    destination = root.expanduser().resolve(strict=False)
    template_path = destination / ".ai-stp-template.json"
    if not template_path.is_file():
        raise _failure("the authoring tree has no scaffold descriptor")
    descriptor = ComponentTemplateDescriptor.model_validate(
        json.loads(template_path.read_text(encoding="utf-8"))
    )
    extra = list(descriptor.additional_harnesses)
    wanted = tuple(harnesses)
    if not wanted:
        raise _failure("a second adaptation requires one concrete harness")
    for harness in wanted:
        if harness == "portable" or harness not in HARNESSES:
            raise _failure("a second adaptation requires one concrete harness")
        if harness == descriptor.harness_variant or harness in extra:
            raise _failure("that harness already has an adaptation in this tree")
        extra.append(harness)
    passport_path = destination / "component-passport.json"
    name = destination.name
    if passport_path.is_file():
        patch = json.loads(passport_path.read_text(encoding="utf-8"))
        held = patch.get("name")
        if isinstance(held, str) and held:
            name = held
    prepared: dict[str, bytes] = {}
    for harness in wanted:
        added = descriptor.model_copy(update={"harness_variant": harness})
        native, _entry, _managed, _kind, _source = _native_source(name, added)
        projection_root = f"projections/{harness}"
        prepared[f"{projection_root}/GENERATED.md"] = (
            b"# Generated projection\n\n"
            b"These native bytes are derived from `source/`. Do not edit them in place.\n"
        )
        prepared.update({f"{projection_root}/{path}": payload for path, payload in native.items()})
    updated = descriptor.model_copy(update={"additional_harnesses": extra})
    template_path.write_bytes(canonize(cast(JsonValue, updated.model_dump(mode="json"))))
    if passport_path.is_file():
        patch = json.loads(passport_path.read_text(encoding="utf-8"))
        raw_variants = patch.get("harness_variants")
        variants: list[str] = []
        if isinstance(raw_variants, list):
            for item in cast(list[object], raw_variants):
                if isinstance(item, str):
                    variants.append(item)
        primary = descriptor.harness_variant
        if primary != "portable" and primary not in variants:
            variants.append(primary)
        for harness in wanted:
            if harness not in variants:
                variants.append(harness)
        patch["harness_variants"] = variants
        passport_path.write_bytes(canonize(cast(JsonValue, patch)))
...
def add_missing_adaptations(root: Path) -> dict[str, bytes]:
    """Add every remaining concrete harness, or refuse if any projection is blocked."""
    import json
    ...
    extra = set(descriptor.additional_harnesses)
    remaining = tuple(
        harness
        for harness in HARNESS_ID_ORDER
        if harness != descriptor.harness_variant and harness not in extra
    )
    if not remaining:
        raise _failure("every concrete harness already has an adaptation in this tree")
    return add_adaptations(destination, remaining)
