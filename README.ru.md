...
`0.0.16` — первая поддерживаемая alpha-линия. `0.0.18` продолжает её как одно
...
точную запись релиза всего estate. `dev` — ветка по умолчанию: рабочие PR входят
в `dev`, затем продвигаются в защищённую `main`; администраторы могут обходить
правила, обязательных approvals нет (`ADR-0180`). Rust и новые виды компонентов отложены;
