...
Прежний эстейт `NDDev-it-com/*-app` снят (`ADR-0119`, `ADR-0120`). Он остаётся в исторических ADR как контекст и не является источником для нового объекта; линия доверия закреплена `provider-policy.toml` за семью `*-setup-system`.
