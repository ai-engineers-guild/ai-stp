...
- `REQ-801`: Провайдеры доставляются версионируемыми артефактами выпуска, а не рабочими подмодулями Git. Установка по протоколу v3 требует подписанного манифеста выпуска или закреплённой GitHub attestation по схеме политики v3; провайдер без того и другого устанавливается только по явному отдельному параметру, и план сообщает такую установку как непроверенную. Изменяющий путь ограничен этим правилом, наблюдающие вызовы провайдера — нет.
...
- `REQ-811`: Выпуск провайдера принимается только после проверки закреплённой политики доверия. Подписанный выпуск проверяет подпись, источник, хэш артефакта, принадлежность закреплённому перечню одобренных выпусков и последовательность защиты от отката. Перечень закрепляет digest вместе с `provider_id` и `repository`, которые вправе его предъявить; пустой перечень не разрешает ничего. Схема политики v3 добавляет отдельный перечень `attested_releases`; такие байты проверяются GitHub artifact attestation против закреплённого репозитория, а не Ed25519-ключом схемы v2.
...
|---|---|
| `REQ-801` | Проверка установки получает провайдер из манифеста выпуска без клонирования Git; `install plan` по протоколу v3 без манифеста отклоняется и называет оба следующих действия, с явным параметром планирует и сообщает `provider_release_trusted` равным `false`, а манифест вместе с этим параметром отклоняется как противоречие; закреплённый attested digest без манифеста планирует и сообщает `provider_release_trusted` равным `true` и `provider_release_trust` равным `github_attestation`; v1 планирует без манифеста как прежде. |
