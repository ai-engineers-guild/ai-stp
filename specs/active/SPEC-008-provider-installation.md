...
- `REQ-802`: All seven closed set providers implement a common versioned protocol core and machine-readable `provider-info`; product-specific software lifecycle and launch are declared capabilities, not fictitious required commands. Each announcement is read from its exact released provider; all seven current providers declare the program lifecycle. Launch is announced only when the product can be pointed at `--target` for every owned surface. Antigravity announces `launch` bound to the documented `~/.gemini` home with an empty `config_home_env`; an alternate root is refused by name. Do not invent `ANTIGRAVITY_*`.
...
- `REQ-846`: The trust level, identity attestation and digest of the used bundle are included in the immutable install plan, re-checked before apply and stored in the local history. The plan-bound manifest selects the delivery verifier: GitHub revalidation feeds `gh attestation verify --bundle` the Sigstore bundle extracted from the saved JSON response, rather than the `attestation` wrapper with `bundle_url`; index revalidation verifies the retained wheel, exact executable, source commit, pinned publisher and PEP 740 provenance matching the approved document. The compatible field `provider_release_trusted` is output as `level != unverified` but is not a level source.
