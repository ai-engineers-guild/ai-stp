...
- `REQ-4104`: The scaffold contains the descriptor as `component.json`, private
  component passport patch, `SetupEvalProfile`, README, safety declaration,
  publication checklist, source scaffold under `source/`, one adaptation
  document under `adaptations/<harness>/`, and a check for the selected language.
...
- `REQ-4108`: The scaffold contains a `projections/<harness>/` catalog with the
  ...
  projection registry; an entire settings file is one component. A Codex `agent`
  is not disguised as a separate type: that combination is rejected.
