...
| `REQ-8011` | `test_cli_task` refuses inspect `--input` facts and round-trips `--input` through the parser; `test_cli_schema_discovery` parses a YAML object into the same facts, refuses duplicate keys and non-object documents, and asserts a schema refusal carries `details.fields`, the `details.errors` pointer/issue/detail list, `details.schema`, and a `cli` continuation to `schema show`. |
