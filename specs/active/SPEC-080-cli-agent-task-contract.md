...
- `REQ-8020`: The twenty agent qualification corpus scenarios have a deterministic argv
  ...
  indexes are refused before effects. The runner also binds both layers to
  `execution_identity`: loaded first-party payload bytes (or mounted checkout
  bytes for Docker), the copied control Skill, initialize prompt, installation
  mode and CLI version. A direct archive digest is recorded when installation
  metadata provides it; its absence is not a release claim. Distribution runs
  copy the packaged Skill. An existing scored overlay with missing or different
  executable identity requires a new measured path. Identity is rechecked before
  scoring, so a source edit during a turn cannot inherit that turn's result.
  Each attempt retains the prompt, model/driver digest, execution identity and
  verdict. Direct attempts require an empty workspace; fill creates unique
  directories and never removes an earlier attempt.
  ...
  and a failed drain is `fail`. Positive `publish-private` and
  `publish-public-filesystem` scores require completed publication, matching
  visibility, no repository binding, and independent online reads of the exact
  version and artifact digest. Setup outcomes use setup routes, component
  outcomes use component routes. Private publication additionally requires an
  anonymous not-found response; an unavailable read is `not_run`, never proof
  of denial. A failed integrity check or anonymous disclosure is `fail`.
  Authorization refusal only satisfies `auth-required-publish`; it does not
  satisfy a positive publication cell. The request recorder remains available
  through independent readback and closes on every exit, including exceptions.
  Publish prompts name a cwd-relative `--input` and do not
