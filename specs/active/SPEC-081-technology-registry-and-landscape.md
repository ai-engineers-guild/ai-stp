...
- `REQ-8201`: Registry records have stable technology/category identifiers,
  ...
  organizational adoption and responsibility decisions owned by SPEC-082.
  ...
  `category` prefixes, not passport object-kind or harness namespaces (ADR-0182).
- `REQ-8202`: Seed/import replay preserves identifiers and manual edits. Initial
...
- `REQ-8204`: Technology lifecycle is `draft`, `active`, `deprecated`, `archived`.
...
- `REQ-8205`: Coordinate mappings are immutable versioned records connecting
...
- `REQ-8206`: Authorized users manually declare, confirm, reject, override,
...
- `REQ-8207`: Evidence records preserve safe source/reference, repository-relative
...
- `REQ-8208`: The versioned #222/#208 handoff names canonical technology identity,
...
- `REQ-8209`: Organization/team/project table and category-grouped views share
...
- `REQ-8210`: Active totals exclude archived, deprecated, deleted, and inactive
...
- `REQ-8211`: CRUD, resolution/search, evidence, counts and drill-down enforce
...
Project lifecycle is owned by SPEC-079 and ADR-0183. Landscape filters use its
...
links, assignments and audit history; SPEC-082 defines identity integration.
...
|---|---|
| `REQ-8201` | Storage/API rename and reclassification tests preserve IDs and references. |
| `REQ-8202` | All category-family fixtures, reserved-kind rejection, Bun and naming distinctions; seed replay preserves owner edits. |
| `REQ-8203` | Normalization, ambiguity, collision and merge tests preserve redirects/history and abort conflicting merges atomically. |
| `REQ-8204` | Authorized/denied lifecycle transition and restoration matrix across API/Web. |
| `REQ-8205` | Versioned mapping refresh tests leave reviewed evidence and decisions unchanged. |
| `REQ-8206` | API/Web manual review matrix covers all states, contexts, unknown versions and ranges. |
| `REQ-8207` | Complete/incomplete repeated-scan and disagreement fixtures preserve owner review and immutable evidence. |
| `REQ-8208` | Detector handoff contract fixtures reject private data, implicit links and component-as-technology publication. |
