...
| `REQ-528` | A table-driven test creates a mustache template for each of the eight types and compares repeated projections for seven specific harnesses; unknown, repeated, and nested conditions, an unsafe path, a symlink, and a size violation fail closed, while tags inside fenced code remain literal. The component scaffold golden asserts `source/` rather than that template. |
