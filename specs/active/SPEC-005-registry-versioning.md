...
- `REQ-510`: `component_type` accepts exactly nine values: `instruction`, `skill`, `mcp`, `hook`, `command`, `agent`, `plugin`, `setting`, `cli`. `command` is a named slash invocation; `cli` is a standalone executable (`ADR-0155`). `marketplace` remains rejected.
...
- `REQ-525`: The owner or their Agent may enrich a local component draft with a private patch against the exact current revision. Each confirmed change creates a content-addressed child revision; a stale revision, secret-bearing field, or unknown field is rejected. A confirmed patch may declare the existing target scope together with the harness and relative managed paths, allowing an observed portable draft to acquire an explicit install route. Local publication-completeness validation lists every missing and invalid field and does not grant cloud authorization.
...
- `REQ-528`: The CLI keeps a safe mustache authoring template for each of the closed component types (`component template render`) and deterministically projects it for one specific harness. Conditional blocks are bound to the closed harness registry, the path-placeholder accepts only a bounded relative POSIX path, and unknown or ambiguous syntax is rejected before publication; tags inside fenced code remain literal text. The complete component scaffold in SPEC-041 is the canonical type-specific `source/` and `projections/<harness>/` tree; the mustache template is not that tree.
...
- `REQ-533`: `component version release` accepts only a publication-complete draft, materializes its native projection into content-addressed storage, and stores a normalized immutable version snapshot separately from the mutable draft head. The recorded `X.Y` points to that snapshot and its exact passport digest; it never points at the draft revision or reconstructs a passport later during publication.
- `REQ-534`: An explicit path is inventoried as that root only. Canonical component and setup passports are classified before native detectors. Generated projections and embedded setup members are distinct from independent objects. Global harness homes are not imported unless the caller asked for global discovery. No stable identifier is minted. Malformed entries are diagnostics. Observation remains separate from adoption (`REQ-518`).
- `REQ-535`: Bounded component discovery and path inventory report `complete` and, when a directory budget stops the walk, an opaque `continuation` of the remaining partition. Repeating with that cursor yields the rest of the same named root without duplicates. The union of pages equals one adequately provisioned scan. An unreadable directory is diagnosed and is not an empty listing. `--cursor` is declared only with a handler that resumes it.
- `REQ-536`: Adoption binds a source path to one stable component identity. Repeating an unchanged path is a no-op. Changing bytes at that path creates a child revision of the same identity. Moving the source to a new path whose old recorded path is gone, and whose head digest still matches, updates that binding rather than minting a new identity. A copy whose original path still exists is a second object. Identical bytes from two live paths are not merged.
...
| `REQ-510` | Enum validation accepts exactly nine values, including `cli`, and rejects `marketplace`. |
...
| `REQ-528` | A table-driven test creates a mustache template for each closed type and compares repeated projections for seven specific harnesses; unknown, repeated, and nested conditions, an unsafe path, a symlink, and a size violation fail closed, while tags inside fenced code remain literal. The component scaffold golden asserts `source/` rather than that template. |
...
| `REQ-533` | A process-level lifecycle refuses release before required authoring metadata exists, then proves the released revision differs from the draft head, references canonical projection bytes, remains unchanged after another draft update, and is the exact revision returned by version listing and publication. |
| `REQ-534` | A mixed authoring fixture returns independent setup and component objects, marks `projections/<harness>/` as generated and setup `components/` as embedded, does not list planted global-home components, does not treat a generated projection directory as an independent skill, repeats byte-identically, mints no `stable_id`, and reports a malformed passport as `invalid_manifest`. |
| `REQ-535` | A portable-skill tree deeper than the directory budget returns `complete=false` and a continuation; resumed pages are disjoint and union to the full scan. An unreadable skill directory is `unreadable` and `complete=false`, not an empty listing. A bounded path-inventory walk resumes the same way. |
| `REQ-536` | Adopting the same source twice keeps one stable id; changing bytes keeps that id with a new revision; renaming the directory keeps the id and updates the binding; copying while the original remains mints a second id. |
...
| `REQ-525` | A process-level scenario passes show/update/validate; a stale expected revision, symlink/oversized patch, secret field, and incomplete exact source produce typed rejections, while a previously released version continues to reference the prior revision. An adopted portable skill can be explicitly declared for a supported user-root harness route and released; an unknown scope is rejected. |
