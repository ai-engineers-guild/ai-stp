...
- **Active context** — either local state or one explicitly selected remote
  organization.
...
- `REQ-7403`: Anonymous/authenticated access is represented separately from
  product mode. Anonymous public catalog reads do not create a fourth mode, and
  authentication alone does not select corporate mode.
...
- `REQ-7409`: The mode matrix is additive by capability family. Enabling a later
  ...
  an organization.
- `REQ-7410`: Switching the active context recomputes capabilities and clears
  context-bound client state. It does not move resources, merge projects, change
  ownership, or reuse cached data from the previous organization.
...
Product mode is not accepted from an untrusted request as authority. For remote
contexts the server derives it from the selected organization and current
membership. Local mode sends no local path, project contents, environment value,
or private passport to determine mode. Logs may record mode and a safe context
identifier but not capability-bearing session material.

## Compatibility and migration

Add product mode and capability fields to new response versions or as optional
fields under the existing additive policy. Existing clients continue to use
their current local, anonymous, or authenticated paths. Deployment profiles keep
their current names and behavior. Rollback removes B2B capability exposure while
leaving local state and personal account data intact.
...
| `REQ-7402` | A context matrix resolves local without auth, personal with the personal organization, and corporate only with active membership. |
...
| `REQ-7410` | Switching between two organizations clears cached scoped data and performs no ownership or project-link mutation. |
