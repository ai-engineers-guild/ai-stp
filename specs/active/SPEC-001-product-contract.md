...
В MVP входят локальный реестр, анонимное чтение публичного реестра, приватная синхронизация после входа, публикация, пять поддерживаемых харнессов и ограниченный режим `undefined`.
...
- **Основная поддержка** — полный сквозной сценарий и release evidence для Claude Code и Codex; блокирует первый выпуск MVP.
- **Бета-поддержка** — тот же безопасный жизненный цикл, но возможна неполная поддержка нативных поверхностей Pi, OpenCode и Grok Build; линия продвигается независимо и первый выпуск не блокирует.
...
- `REQ-105`: Поддерживаются Claude Code, Codex, Pi, OpenCode, Grok Build и ограниченный идентификатор `undefined`.
- `REQ-106`: Claude Code и Codex проходят основной сквозной сценарий; Pi, OpenCode и Grok Build проходят бета-сценарий без ослабления plan, backup и restore.
...
- `REQ-113`: Пять харнессов являются полным набором поддержки MVP; новый официальный `harness_id` появляется только через платформенный процесс продвижения по `ADR-0033`, а пользовательские адаптеры не публикуются как официальная поддержка.
...
| `REQ-105` | Enum/schema test принимает ровно пять идентификаторов и `undefined`. |
...
| `REQ-113` | Проверка перечисления отклоняет седьмое значение без нового ADR и версии схемы, а инвентаризация не находит пути публикации пользовательского адаптера как официальной поддержки. |
