...
- `REQ-631`: A component is compatible only through an exact adaptation for the selected harness and scope. A provider-native route is checked after that adaptation exists and never creates compatibility; absence is `adaptation_unavailable`, while an explicit adaptation whose required provider surface is absent or unusable on a measured native version is `provider_surface_unavailable`.
- `REQ-633`: Provider-native kind may differ from the logical component type only when the adaptation declares the mapping, its exact generated projection preserves the logical function, current verification covers that mapping, and `semantic_losses` is empty. Otherwise compilation is blocked.
- `REQ-632`: A bundle and an installation plan are compiled for one projection scope chosen by the caller — `global`, the harness configuration home; `project`, the workspace named by the plan's target; or `user_root`, the shared `~/.agents` root named by the plan's target — and a component is routed to the provider's native surface for its kind at that scope; a kind the provider declares at no such scope is `unsupported` there rather than routed to another scope's surface, a `global` compile falls back to a `user_root` rule only where the home has no rule of that kind, a `project` or `user_root` plan requires an explicit target, and the compiled bundle records a non-default scope in its manifest.
...
| `REQ-631` | A provider route without an exact component adaptation is refused with `adaptation_unavailable`; an exact adaptation whose declared provider-native surface is missing is refused with `provider_surface_unavailable`. Antigravity CLI 1.2.10 legacy commands refuse even with task authority, while skills and other measured version inputs retain their ordinary checks. |
| `REQ-633` | Fixtures cover MCP implemented as a native setting, file or package with exact adaptation declarations and refuse implicit conversion, stale mapping evidence and every non-empty semantic-loss set. |
| `REQ-632` | An antigravity skill compiled with `--scope project` lands under `.agents/skills` and the plan carries `--target-scope project`; a pi skill compiled with `--scope user_root` lands under `skills` of the shared root with `--target-scope user_root` while the same skill compiled without a scope lands under the pi home's `skills`; a codex skill compiled without a scope still reaches the shared root, the only place codex declares; `install plan --scope project` or `--scope user_root` without `--target` is refused before any provider runs; an antigravity instruction compiled for `project` is `unsupported`, not routed to `config/rules`. |
