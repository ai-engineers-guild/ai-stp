...
- `REQ-632`: A bundle and an installation plan are compiled for one projection scope chosen by the caller — `global`, the harness configuration home, or `project`, the workspace named by the plan's target — and a component is routed to the provider's native surface for its kind at that scope; a kind the provider declares at no such scope is `unsupported` there rather than routed to another scope's surface, a `project` plan requires an explicit target, and the compiled bundle records a non-default scope in its manifest.
...
| `REQ-632` | An antigravity skill compiled with `--scope project` lands under `.agents/skills` and the plan carries `--target-scope project`; the same skill compiled without a scope lands under `config/skills` with a byte-identical manifest to one compiled before scopes existed; `install plan --scope project` without `--target` is refused before any provider runs; an antigravity instruction compiled for `project` is `unsupported`, not routed to `config/rules`. |
