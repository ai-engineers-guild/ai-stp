...
| `REQ-631` | A provider route without an exact component adaptation is refused with `adaptation_unavailable`; an exact adaptation whose declared provider-native surface is missing is refused with `provider_surface_unavailable`. |
