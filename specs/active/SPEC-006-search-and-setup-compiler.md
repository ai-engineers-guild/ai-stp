...
- `REQ-631`: A component that names no harness is compatible with every harness whose provider declares a native surface for its kind, and is refused with `provider_surface_unavailable` where none does; `harness_mismatch` is reserved for an object that declares another harness.
...
| `REQ-631` | A portable instruction component adopted from a repository-root `AGENTS.md` is admissible for a harness with an instruction surface and refused with `provider_surface_unavailable`, never `harness_mismatch`, for one without. |
