...
Запускается через just, а не напрямую:
    ...
    just docs-static    проверить
    just docs-gen       пересобрать таблицы в index.md
...
# Корни, к которым правила индексов и frontmatter неприменимы: там проверяются
# только целостность ссылок и разметка. Сейчас таких корней нет; список
# сохранён, потому что от него зависят needs_frontmatter и sync_index_tables.
...
#: Документ, названный в обратных кавычках. Только Markdown: прочие имена в
#: обратных кавычках описывают будущие артефакты, а не файлы репозитория.
...
def tracked_markdown(root: Path) -> list[Path]:
    """Markdown, который репозиторий содержит: отслеживаемый плюс новый локальный.

    Неотслеживаемые файлы, не объявленные в ignore, включены намеренно: новый
    документ обязан проходить проверки до первого `git add`, а не после.
    """
    try:
        ...
        # Вне рабочего дерева Git обход остаётся единственным источником.
...
class Linter:
    ...
    def needs_frontmatter(self, path: Path) -> bool:
        """Frontmatter несут документы, а не короткоживущие рабочие артефакты.

        Корень из STRUCTURE_EXEMPT описывает себя только индексом: остальные его
        файлы живут недолго, и описывать каждый для индекса незачем.
        """
        if path.parent == ROOT:
            return False
    ...
    def check_frontmatter(self, path: Path, text: str) -> None:
        ...
        if not m:
            self.error(path, 1, "FM001", "нет frontmatter с description и last_verified")
            return
        ...
        if extra:
            self.error(
                ...
                f"Разрешены только description и last_verified",
            )
    ...
    def check_freshness(self, path: Path, value: str) -> None:
        ...
        if PLACEHOLDER_RE.search(value):
            if not self.template_mode:
                self.error(path, 3, "FR001", "last_verified не заполнен")
            return
        try:
            verified = dt.date.fromisoformat(value.strip())
        except ValueError:
            self.error(path, 3, "FR002", f"last_verified не является датой ГГГГ-ММ-ДД: {value}")
            return
        age = (self.current_date - verified).days
        if age < 0:
            self.error(path, 3, "FR003", f"last_verified в будущем: {value}")
    ...
    def check_backticked_documents(self, path: Path, text: str) -> None:
        ...
        check_links проверяет ссылки Markdown и не видит имя документа, поданное
        обратными кавычками. Именно так пережили две висячие ссылки:
        cli-api-contract в ADR-0013 и
        contracts/component-setup-manifests.md в ADR-0012 — оба документа были
        свёрнуты, а упоминания остались и читались как действующие.

        Проверяется только расширение .md: остальные упоминания в обратных
        кавычках описывают будущие артефакты вроде bundle.json, названные
        контрактом, а не файлы репозитория.
        """
        for line_no, line in iter_lines_outside_fences(text):
            for match in BACKTICKED_DOCUMENT_RE.finditer(line):
                ...
                if reference.startswith("/"):
                    # Ведущий слэш означает адрес, который отдаёт веб, а не файл
                    # этого дерева: `/agents.md` стоит в одном ряду с `/llms.txt`
                    # и принадлежит `SPEC-036`. Проверять его как путь нельзя ещё
                    # и потому, что `path.parent / "/agents.md"` в pathlib уходит
                    # в корень файловой системы, где одноимённый файл молча
                    # удовлетворил бы правило.
                    continue
                candidates = (
                ...
                if any(candidate.exists() for candidate in candidates):
                    continue
                # Документы законно называют соседей одним именем файла, а
                # CLAUDE.md и SKILL.md — реальные файлы, живущие не в docs/.
                # Проверяется существование, а не расположение: цель правила —
                # поймать имя, за которым не стоит ничего.
                ...
                self.error(
                    ...
                    line_no,
                    "LN002",
                    f"документ в обратных кавычках не существует: {reference}",
                )

    def document_names(self) -> set[str]:
        """Имена всех Markdown-файлов репозитория, построенные один раз.

        Список даёт Git, а не обход дерева. Правило LN002 отвечает на вопрос
        «за этим именем что-то стоит», и сборочный вывод отвечал на него за
        репозиторий: `.next` создаёт каталог `agents.md`, из-за чего ссылка на
        несуществующий документ проходила молча, стоило один раз собрать веб.
        Список исключений пришлось бы дополнять под каждый новый инструмент
        сборки, тогда как Git уже знает, что принадлежит репозиторию.
        """
    ...
    def sync_index_tables(self) -> None:
        ...
        """
        for root in DOC_ROOTS:
            # Освобождённый корень ведёт собственную доску вместо перечня
            # документов, поэтому таблица содержимого для него не собирается.
            ...
            for index in [base / "index.md", *base.rglob("index.md")]:
                ...
                if self.fix:
                    ...
                    self.error(
                        ...
                        1,
                        "IX005",
                        "таблица содержимого разошлась с папкой. Запусти just docs-gen",
                    )
    ...
    def check_adr_identities(self) -> None:
        ...
        for path in sorted(base.glob("ADR-*.md")):
            ...
            if match is None:
                self.error(path, 1, "AD001", "имя ADR не соответствует ADR-NNNN-kebab-case.md")
                continue
            ...
            if previous is not None:
                self.error(
                    ...
                    1,
                    "AD002",
                    f"номер ADR-{identifier} уже принадлежит {previous.name}",
                )
            else:
            ...
            if heading is None:
                self.error(path, 1, "AD003", "нет заголовка вида # ADR-NNNN: решение")
        ...
        for line_no, line in iter_lines_outside_fences(text):
            if CHECKBOX_RE.match(line):
                self.error(
                    ...
                    line_no,
                    "ST001",
                    "чек-лист в docs/. Состояние задач живёт в трекере, а не в документации",
                )
                return
