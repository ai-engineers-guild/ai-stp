...
# Every value a serving container refuses to start without is checked here —
# before build, migration or any recreation, so a missing or placeholder
# secret leaves the currently healthy release serving traffic. Nothing inside
# sources or prints the environment file; `just infra-env-check` rehearses
