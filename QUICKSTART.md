...
Источник истины Python-зависимостей один — корневой `uv.lock`; документационные инструменты живут группой `docs`, инструменты разработки — группой `dev`. Node-инструменты закреплены в `docs_scripts/bun.lock` и ставятся `bun`. Произвольные глобальные версии линтеров не используются.
