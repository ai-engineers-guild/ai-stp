...
Если rollback сообщает, что предыдущий CLI не читает текущую схему registry,
оставьте совместимый CLI. Отказ сохраняет текущие данные; восстановление
резервной копии до обновления потеряло бы последующую работу и не входит
в откат программы.
