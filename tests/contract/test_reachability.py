...
def test_every_command_returns_the_model_its_declared_schema_names() -> None:
    ...
    from ai_stp_cli.registry import COMMANDS
    from ai_stp_contracts.schemas import CLI_MODELS, HTTP_MODELS

    # A CLI command may deliberately return a shared HTTP wire model (for
    # example, the GitHub connector status).  Resolve declarations across both
    # published registries without making the HTTP boundary a CLI payload.
    declared_models = {**HTTP_MODELS, **CLI_MODELS}
    ...
    for command in COMMANDS:
        ...
        if declared_models.get(name) is not inside[0]:
            mismatched.append(
